...
#########################################################################################################################
################### 브라우저를 켜고 검색 키워드 입력 ###################################################################
querys= {"삼성전자":'005930','하이닉스':'000660','LG화학':'051910','현대차':'005380','셀트리온':'068270'}
...
###### 날짜 저장 ##########
## 현재 시간
days_li=['mon','tus','wed','thr','fri','sat','sun']
b = datetime.today().weekday()
if days_li[b] =='mon':
    a=str(datetime.now()-timedelta(days=3))
else:
...
for press_nm in press_nms:
    for query,cd in querys.items():
        ...
        news_url = 'https://search.naver.com/search.naver?where=news&query={}&sm=tab_opt&sort=0&photo=0& \
            field=0&pd=3&ds={}&de={}&start=1'.format(query,20210924,20210926)
